...
const usePromise = (promiseFn, immediate = true) => {
  // All States necessary for the promise.
  const [data, setData] = useState(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(null);

  // the function that executes the promise.
  const execute = useCallback(
    ...
    [setData, setIsLoading, setError, promiseFn]
  ); // re-run if setData, setIsLoading, setError or promiseFn changes.
